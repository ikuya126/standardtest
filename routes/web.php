<?php

use Illuminate\Support\Facades\Route;
use App\Http\Controllers\ContactController;

/*
|--------------------------------------------------------------------------
| Web Routes
|--------------------------------------------------------------------------
|
| Here is where you can register web routes for your application. These
| routes are loaded by the RouteServiceProvider within a group which
| contains the "web" middleware group. Now create something great!
|
*/

Route::get('/',[ContactController::class,'index']);
Route::post('/form',[ContactController::class,'post'])->name("form");
Route::get('/confirm',[ContactController::class,'confirm'])->name('confirm');
Route::post('/create',[ContactController::class,'create'])->name('create');
Route::get('/thanks',[ContactController::class,'thanks'])->name('thanks');
Route::get('/managepage',[ContactController::class,'managepage']);
Route::post('/manage',[ContactController::class,'manage'])->name('manage');
Route::delete('/managepage/{id}',[ContactController::class,'delete'])->name('delete');